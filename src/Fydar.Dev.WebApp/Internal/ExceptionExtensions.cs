...
{
	private static readonly Regex lineInterpreter = GeneratedRegex();
		...
		StringBuilder stringBuilder)
	{
		...
		if (stackTrace != null)
		{
			string[] stackTraceLines = stackTrace.Split(lineSeperators, StringSplitOptions.RemoveEmptyEntries);
	...
	private static string FormatStackLine(
		string line)
	{
		...
		if (match.Success)
		{
			...
			string typeName = method[..methodSeperator];
		...
		{
			line = line[..removeAfterIndex];
		}

		return line;
	}

	[GeneratedRegex("^(at .*?) in (.*):line (\\d+)$")]
